...
class SEResNeXtBottleneck(Bottleneck):
    ...
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
                 downsample=None, base_width=4, norm_layer=nn.BatchNorm2d):
...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
                 ...
                 downsample_padding=1, num_classes=1000, norm_layer=nn.BatchNorm2d):
        """
        Parameters
        ----------
    ...
    def _make_layer(self, block, planes, blocks, groups, reduction, stride=1,
                    downsample_kernel_size=1, downsample_padding=0, norm_layer=nn.BatchNorm2d):
        downsample = None
        ...
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups, reduction, norm_layer=norm_layer))
...
def remove_fc(state_dict):
    return {key: value for key, value in state_dict.items() if not key.startswith('last_linear')}
...
def se_resnext101_32x4d(num_classes=1000, pretrained='imagenet', norm_layer=nn.BatchNorm2d):
